...
import { adminApi, type CmsPage } from '@/lib/api';
...
function filterPages(pages: CmsPage[], key: FilterKey): CmsPage[] {
  ...
  if (key === 'diger') {
    return pages.filter(
      (p) =>
        !p.slug.startsWith('hakkimizda') &&
        !p.slug.startsWith('uye-ol') &&
        !p.slug.startsWith('meslegin-gelecekleri'),
    );
  }
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Hakkımızda, İletişim, Mesleğin Gelecekleri vb. içerik sayfaları
          </p>
        </div>
        <Link
          href="/sayfalar/yeni"
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              filter === f.key
                ...
                : 'bg-white border border-gray-200 text-gray-600 hover:border-[var(--color-mavi)] hover:text-[var(--color-mavi)]'
            }`}
          >
            {f.label}
            {f.key !== 'tumü' && (
              <span className="ml-1 opacity-70">
                ({filterPages(pages, f.key).length})
              </span>
            )}
          </button>
        ))}
                ...
                <tr key={page.slug} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <Link
                      ...
                      className="text-[var(--color-mavi)] hover:underline text-xs font-medium"
                    >
                      Düzenle
                    </Link>
                    <button
