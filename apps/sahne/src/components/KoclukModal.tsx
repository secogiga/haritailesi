...
const SINAVLAR = ['KPSS', 'Gayrimenkul Değerleme', 'İHA Sertifikası'];
...
export default function KoclukModal({ open, defaultSinav, onClose }: Props) {
  const [sinav, setSinav] = useState(defaultSinav ?? SINAVLAR[0]);
  const [form, setForm] = useState({ ad: '', email: '', telefon: '', not: '' });
  ...
  useEffect(() => { if (defaultSinav) setSinav(defaultSinav); }, [defaultSinav]);

  useEffect(() => {
    if (!open) {
      setStep('form');
      setForm({ ad: '', email: '', telefon: '', not: '' });
      setDigits(['', '', '', '', '', '']);
      setCodeError(false);
      setCountdown(60);
  ...
  const handleVerify = () => {
    ...
    // gerçek API bağlantısı buraya gelir; şimdilik her 6 haneli kod geçerli
    setStep('success');
    if (timerRef.current) clearInterval(timerRef.current);
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div style={{ background: 'linear-gradient(160deg,#0f172a 0%,#1e3a8a 100%)', padding: '24px 28px 20px' }}>
          <button
            onClick={onClose}
          ...
          <div className="text-white font-black text-lg leading-snug mb-1">
            {step === 'verify' ? 'Kimliğinizi Doğrulayın' : step === 'success' ? 'Talebiniz Alındı!' : 'Koçluk Hizmeti Talebi'}
          </div>
          <div className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
            {step === 'verify'
              ? `${channel === 'email' ? 'E-posta' : 'Telefon'} adresinize 6 haneli bir doğrulama kodu gönderdik.`
              : step === 'success'
              ? 'Koçluk ekibimiz en geç 1 iş günü içinde sizinle iletişime geçecek.'
              : 'Bilgilerinizi bırakın, uzman koçumuz sizi arasın.'}
          </div>
        </div>
        ...
        <div className="p-7">
          ...
          {step === 'form' && (
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">Hangi Sınav?</label>
                <div className="flex gap-2 flex-wrap">
                  {SINAVLAR.map(s => (
                    <button key={s} type="button" onClick={() => setSinav(s)}
                      className="text-xs font-bold px-3 py-1.5 rounded-lg border transition-all"
                      style={{ background: sinav === s ? '#0b1829' : '#fff', color: sinav === s ? '#fff' : '#6b7280', borderColor: sinav === s ? '#0b1829' : '#e5e7eb' }}
                    >{s}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider">
                  Notunuz <span className="font-normal normal-case text-gray-400">(isteğe bağlı)</span>
                </label>
                <textarea rows={3} placeholder="Sınav tarihiniz, hedef puanınız veya özel durumunuz…"
                  value={form.not} onChange={e => setForm(f => ({ ...f, not: e.target.value }))}
                  className="w-full text-sm text-gray-800 border border-gray-200 rounded-xl px-3.5 py-2.5 outline-none focus:border-gray-800 transition-colors resize-none" />
              </div>

              <button type="submit" disabled={!form.ad || (!form.email && !form.telefon)}
          ...
          {step === 'verify' && (
            <div className="flex flex-col gap-5">
                  ...
                  <div className="flex gap-2">
                    {(['email', 'tel'] as Channel[]).map(ch => (
                      <button key={ch} type="button" onClick={() => setChannel(ch)}
                        ...
                        style={{ background: channel === ch ? '#0b1829' : '#fff', color: channel === ch ? '#fff' : '#6b7280', borderColor: channel === ch ? '#0b1829' : '#e5e7eb' }}
                      >
              ...
              <div>
                ...
                <div className="flex gap-2 justify-between" onPaste={handlePaste}>
                  {digits.map((d, i) => (
                    <input
                      ...
                      style={{
                        borderColor: codeError ? '#ef4444' : d ? '#0b1829' : '#e5e7eb',
                        background: d ? '#f8faff' : '#fff',
                        color: '#0b1829',
                      }}
                    />
                  ))}
                ...
                {codeError && (
                  <p className="text-xs text-red-500 mt-2">Lütfen 6 haneli kodu eksiksiz girin.</p>
                )}
              </div>
              ...
              <div className="text-center text-xs text-gray-400">
                {countdown > 0
                  ...
                  : <button onClick={resend} className="font-bold text-blue-500 hover:text-blue-700 transition-colors">Kodu Tekrar Gönder</button>
                }
              </div>
          ...
          {step === 'success' && (
            <div className="text-center py-4">
              ...
              <div className="text-sm text-gray-500 leading-relaxed mb-6">
                <span className="font-bold text-[#0b1829]">{form.ad}</span>, koçluk ekibimiz en geç 1 iş günü içinde
                {channel === 'email' ? ` ${form.email}` : ` ${form.telefon}`} üzerinden sizinle iletişime geçecek.
              </div>
