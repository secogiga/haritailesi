...
import type { CmsEvent } from '@/lib/site-admin-api';
...
const EMPTY: Partial<CmsEvent> = {
  ...
  registrationUrl: '', meetingUrl: '', maxCapacity: undefined,
...
export default function EtkinliklerPage() {
  ...
  const loadEvents = useCallback(() => {
    setLoading(true);
    api.listEvents(typeFilter ? { type: typeFilter } : undefined)
  ...
  function openEdit(ev: CmsEvent) {
    ...
    setForm({
      ...
      maxCapacity: ev.maxCapacity ?? undefined,
  ...
  async function handleSave() {
    ...
    try {
      if (editItem) {
        await api.updateEvent(editItem.id, form);
      } else {
        if (!form.slug?.trim()) { setFormError('Slug zorunlu.'); setSaving(false); return; }
  ...
  async function handleDelete(id: string, title: string) {
    ...
    setDeleting(id);
    try { await api.deleteEvent(id); setEvents(p => p.filter(e => e.id !== id)); }
  ...
  async function togglePublish(ev: CmsEvent) {
    try {
      const updated = await api.updateEvent(ev.id, { isPublished: !ev.isPublished });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate}
          className="px-4 py-2 bg-[#2d6b68] text-white text-sm font-semibold rounded-lg hover:bg-[#235552] transition-colors">
          + Etkinlik Ekle
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        <button onClick={() => setTypeFilter('')}
          className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${!typeFilter ? 'bg-[#2d6b68] text-white border-[#2d6b68]' : 'bg-white text-gray-600 border-gray-200 hover:border-[#2d6b68]/40'}`}>
          Tümü
        </button>
        {Object.entries(TYPES).map(([k, v]) => (
          <button key={k} onClick={() => setTypeFilter(typeFilter === k ? '' : k)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${typeFilter === k ? 'bg-[#2d6b68] text-white border-[#2d6b68]' : 'bg-white text-gray-600 border-gray-200 hover:border-[#2d6b68]/40'}`}>
            {v}
          </button>
        ))}
      </div>
            ...
            <div key={ev.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-[#2d6b68]/10 text-[#2d6b68]">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => openEdit(ev)}
                  className="p-1.5 rounded-lg text-gray-400 hover:text-[#2d6b68] hover:bg-gray-100 transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button onClick={() => void handleDelete(ev.id, ev.title)} disabled={deleting === ev.id}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 my-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {!editItem && (
                <div className="col-span-2">
                  ...
                  <input className={inp} value={form.slug ?? ''} placeholder="ornek-etkinlik-2025" onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} />
                </div>
              )}
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Tür</label>
                <select className={inp} value={form.type ?? 'webinar'} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}>
              ...
              <div>
                ...
                <input type="datetime-local" className={inp} value={form.dateStart ?? ''} onChange={e => setForm(f => ({ ...f, dateStart: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Bitiş</label>
                <input type="datetime-local" className={inp} value={form.dateEnd ?? ''} onChange={e => setForm(f => ({ ...f, dateEnd: e.target.value }))} />
              </div>
              <div className="col-span-2">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Kayıt URL</label>
                <input className={inp} value={form.registrationUrl ?? ''} onChange={e => setForm(f => ({ ...f, registrationUrl: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Online Toplantı URL</label>
                <input className={inp} value={form.meetingUrl ?? ''} onChange={e => setForm(f => ({ ...f, meetingUrl: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Kapasite</label>
                <input type="number" className={inp} value={form.maxCapacity ?? ''} onChange={e => setForm(f => ({ ...f, maxCapacity: e.target.value ? Number(e.target.value) : undefined }))} />
              </div>
              <div className="flex items-center gap-4 pt-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isPublished ?? false} onChange={e => setForm(f => ({ ...f, isPublished: e.target.checked }))} className="rounded" />
                  <span className="text-sm text-gray-700">Yayınla</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isCancelled ?? false} onChange={e => setForm(f => ({ ...f, isCancelled: e.target.checked }))} className="rounded" />
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button onClick={() => setShowForm(false)}
                className="px-4 py-2 text-sm rounded-xl border border-gray-200 hover:bg-gray-50 transition-colors">İptal</button>
