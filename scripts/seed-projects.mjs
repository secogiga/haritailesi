#!/usr/bin/env node
/**
 * SAHNE_PINNED projelerini DB'ye ekler.
 * Kullanım: node scripts/seed-projects.mjs
 * API'nin çalışıyor olması gerekir: http://localhost:3000
 */
...
async function main() {
  ...
  // 2. Mevcut projeleri listele + sil
  console.log('🗑️  Mevcut projeler siliniyor…');
  const listRes = await fetch(`${API}/admin/cms/projects`, { headers });
  if (!listRes.ok) throw new Error(`Projeler listelenemedi: ${listRes.status}`);
  ...
  for (const proj of PROJECTS) {
    ...
    if (!res.ok) {
      ...
      console.log(`   ✓ Eklendi: ${created.title} (${created.id})`);
    }
  }
